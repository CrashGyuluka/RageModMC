...
public class NightcommCommand {
	...
	public static void registerCommands(RegisterCommandsEvent event) {
		event.getDispatcher()
				.register(Commands.literal("night").requires(s -> s.hasPermission(2))
						.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(NightcommCommand::execute))
						.executes(NightcommCommand::execute));
	}
	...
	private static int execute(CommandContext<CommandSourceStack> ctx) {
		...
		NightcommCommandExecutedProcedure.execute(world, x, y, z);
		return 0;
	}
}
