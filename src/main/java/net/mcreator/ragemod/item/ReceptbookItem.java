...
import net.mcreator.ragemod.item.inventory.ReceptbookInventoryCapability;
...
public class ReceptbookItem extends Item {
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Recept Könyv (Prot)");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new CrashchestguiMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new ReceptbookInventoryCapability();
	}
