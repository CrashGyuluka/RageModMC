...
public class Alienspike5Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienspike5Feature FEATURE = (Alienspike5Feature) new Alienspike5Feature().setRegistryName("ragemod:alienspike_5");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_spike5"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 750000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(9) + 3;
