...
public class MagentaCaveairStrFeature extends Feature<NoneFeatureConfiguration> {
	public static final MagentaCaveairStrFeature FEATURE = (MagentaCaveairStrFeature) new MagentaCaveairStrFeature()
			.setRegistryName("ragemod:magenta_caveair_str");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public MagentaCaveairStrFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "aa_magenta_caveair_str"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 4269) {
