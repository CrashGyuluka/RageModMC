...
public class MarokriksaaliendimensionFeature extends Feature<NoneFeatureConfiguration> {
	public static final MarokriksaaliendimensionFeature FEATURE = (MarokriksaaliendimensionFeature) new MarokriksaaliendimensionFeature()
			.setRegistryName("ragemod:marokriksaaliendimension");
	...
	public MarokriksaaliendimensionFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "marokriksatree01"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 10000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(13) + 4;
