...
public class Ragefabiome1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Ragefabiome1Feature FEATURE = (Ragefabiome1Feature) new Ragefabiome1Feature().setRegistryName("ragemod:ragefabiome_1");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "rage1"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(4) + 2;
