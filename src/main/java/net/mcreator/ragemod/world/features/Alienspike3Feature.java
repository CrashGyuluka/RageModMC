...
public class Alienspike3Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienspike3Feature FEATURE = (Alienspike3Feature) new Alienspike3Feature().setRegistryName("ragemod:alienspike_3");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_spike3"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 750000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(7) + 3;
