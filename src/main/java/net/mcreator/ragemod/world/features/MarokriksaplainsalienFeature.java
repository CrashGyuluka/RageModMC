...
public class MarokriksaplainsalienFeature extends Feature<NoneFeatureConfiguration> {
	public static final MarokriksaplainsalienFeature FEATURE = (MarokriksaplainsalienFeature) new MarokriksaplainsalienFeature()
			.setRegistryName("ragemod:marokriksaplainsalien");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = Set.of(new ResourceLocation("ragemod:alien_plains"));
	private final List<Block> base_blocks = List.of(RagemodModBlocks.ALIEN_SOIL);
	private StructureTemplate template = null;

	public MarokriksaplainsalienFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "marokriksa_alien"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 69420) {
