...
public class Glowingforestpafrany2Feature extends Feature<NoneFeatureConfiguration> {
	public static final Glowingforestpafrany2Feature FEATURE = (Glowingforestpafrany2Feature) new Glowingforestpafrany2Feature()
			.setRegistryName("ragemod:glowingforestpafrany_2");
	...
	public Glowingforestpafrany2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "glowing_paf2"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 800000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(8) + 2;
