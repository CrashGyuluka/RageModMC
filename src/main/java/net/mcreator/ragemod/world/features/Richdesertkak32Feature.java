...
public class Richdesertkak32Feature extends Feature<NoneFeatureConfiguration> {
	public static final Richdesertkak32Feature FEATURE = (Richdesertkak32Feature) new Richdesertkak32Feature()
			.setRegistryName("ragemod:richdesertkak_32");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "a_rich_desert_kak3"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!Richdesertkak1AdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
