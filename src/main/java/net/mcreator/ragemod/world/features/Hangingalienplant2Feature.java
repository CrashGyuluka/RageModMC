...
public class Hangingalienplant2Feature extends Feature<NoneFeatureConfiguration> {
	public static final Hangingalienplant2Feature FEATURE = (Hangingalienplant2Feature) new Hangingalienplant2Feature()
			.setRegistryName("ragemod:hangingalienplant_2");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "hanging_alien_plant"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!Hangingalienplant2AdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
