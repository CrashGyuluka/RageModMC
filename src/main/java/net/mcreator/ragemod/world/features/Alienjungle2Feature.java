...
public class Alienjungle2Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienjungle2Feature FEATURE = (Alienjungle2Feature) new Alienjungle2Feature().setRegistryName("ragemod:alienjungle_2");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_jungle_5"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 600000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(3) + 4;
