...
public class Fa4Feature extends Feature<NoneFeatureConfiguration> {
	public static final Fa4Feature FEATURE = (Fa4Feature) new Fa4Feature().setRegistryName("ragemod:fa_4");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private final List<Block> base_blocks = List.of(Blocks.GRASS_BLOCK, Blocks.STONE);
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "robtai1"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 2000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(4) + 6;
