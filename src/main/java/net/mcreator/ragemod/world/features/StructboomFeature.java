...
public class StructboomFeature extends Feature<NoneFeatureConfiguration> {
	public static final StructboomFeature FEATURE = (StructboomFeature) new StructboomFeature().setRegistryName("ragemod:structboom");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public StructboomFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "struct_boom"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 40) {
