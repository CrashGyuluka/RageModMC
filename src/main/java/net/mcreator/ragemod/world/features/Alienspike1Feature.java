...
public class Alienspike1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienspike1Feature FEATURE = (Alienspike1Feature) new Alienspike1Feature().setRegistryName("ragemod:alienspike_1");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_spike1"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 750000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(6) + 2;
