...
public class SavstructFeature extends Feature<NoneFeatureConfiguration> {
	public static final SavstructFeature FEATURE = (SavstructFeature) new SavstructFeature().setRegistryName("ragemod:savstruct");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public SavstructFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "struct_sav"));
		if (template == null)
			return false;
