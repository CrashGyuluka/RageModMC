...
public class AlienspikeblockstrFeature extends Feature<NoneFeatureConfiguration> {
	public static final AlienspikeblockstrFeature FEATURE = (AlienspikeblockstrFeature) new AlienspikeblockstrFeature()
			.setRegistryName("ragemod:alienspikeblockstr");
	...
	public AlienspikeblockstrFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_spikeblock1"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 100000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(9) + 1;
