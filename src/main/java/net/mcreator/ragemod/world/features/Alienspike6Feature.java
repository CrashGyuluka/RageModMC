...
public class Alienspike6Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienspike6Feature FEATURE = (Alienspike6Feature) new Alienspike6Feature().setRegistryName("ragemod:alienspike_6");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_spike6"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 600000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(7) + 2;
