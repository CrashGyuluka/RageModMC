...
public class Alienjugle1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Alienjugle1Feature FEATURE = (Alienjugle1Feature) new Alienjugle1Feature().setRegistryName("ragemod:alienjugle_1");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_jungle_4"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 600000) {
			boolean anyPlaced = false;
			int count = context.random().nextInt(2) + 4;
