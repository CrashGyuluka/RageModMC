...
public class StructlevFeature extends Feature<NoneFeatureConfiguration> {
	public static final StructlevFeature FEATURE = (StructlevFeature) new StructlevFeature().setRegistryName("ragemod:structlev");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public StructlevFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "struct_lev"));
		if (template == null)
			return false;
