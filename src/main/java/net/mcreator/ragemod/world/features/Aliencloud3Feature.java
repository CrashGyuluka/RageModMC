...
public class Aliencloud3Feature extends Feature<NoneFeatureConfiguration> {
	public static final Aliencloud3Feature FEATURE = (Aliencloud3Feature) new Aliencloud3Feature().setRegistryName("ragemod:aliencloud_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private final List<Block> base_blocks = List.of(Blocks.AIR);
	private StructureTemplate template = null;

	public Aliencloud3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("ragemod", "alien_cloud3"));
		if (template == null)
			return false;
