...
public class Dimension1Dimension extends RagemodModElements.ModElement {
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0, 0.086274509804, 0.086274509804);
			}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(ObsontiumparticleParticle.particle, px, py, pz, vx, vy, vz);
			}
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == ObsontiumblockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? ObsontiumblockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, ObsontiumblockBlock.block.getDefaultState(), 3);
					}
				}
			}
