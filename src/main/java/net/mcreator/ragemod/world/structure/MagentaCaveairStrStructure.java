...
public class MagentaCaveairStrStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (!dimensionCriteria)
						return false;
					if ((random.nextInt(1000000) + 1) <= 4269) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("ragemod", "aa_magenta_caveair_str"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("magenta_caveair_str"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("ragemod:magenta_caveair_str"), configuredFeature);
		}
	}
