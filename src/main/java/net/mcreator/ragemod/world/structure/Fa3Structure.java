...
public class Fa3Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (!dimensionCriteria)
						return false;
					if ((random.nextInt(1000000) + 1) <= 400) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("ragemod", "kizukra_with"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("fa_3"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("ragemod:fa_3"), configuredFeature);
		}
	}
	@SubscribeEvent
