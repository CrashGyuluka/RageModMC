...
public class Hangingalienplant2Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 1000000) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!Hangingalienplant2AdditionalGenerationConditionProcedure.executeProcedure(Stream
			...
			event.getRegistry().register(feature.setRegistryName("hangingalienplant_2"));
