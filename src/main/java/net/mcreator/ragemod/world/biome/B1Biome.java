...
public class B1Biome extends RagemodModElements.ModElement {
	public static Biome biome;
	public B1Biome(RagemodModElements instance) {
		super(instance, 420);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-16750849).setWaterColor(-10066432).setWaterFogColor(-3407872)
						.withSkyColor(-16750849).withFoliageColor(-6711040).withGrassColor(-10092544).build();
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.EVOKER, 1, 1, 2));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0.2f).temperature(0.6f)
						.downfall(0.9f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("ragemod:b_1"));
			}
		}
	}
	@Override
