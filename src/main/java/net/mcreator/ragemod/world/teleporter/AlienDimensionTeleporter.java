...
public class AlienDimensionTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("alien_dimension_portal_igniter_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("alien_dimension_portal_igniter_portal",
				Sets.newHashSet(ImmutableSet.copyOf(RagemodModBlocks.ALIEN_DIMENSION_PORTAL_IGNITER_PORTAL.getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("alien_dimension_portal_igniter_portal");
	...
	public AlienDimensionTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
			...
			for (int i3 = -1; i3 < 2; ++i3) {
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0 ? RagemodModBlocks.CVEX_BLOCK.defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, RagemodModBlocks.CVEX_BLOCK.defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = RagemodModBlocks.ALIEN_DIMENSION_PORTAL_IGNITER_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getPortalRepositioner(entity, blockpos1).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = AlienDimensionPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return AlienDimensionPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
