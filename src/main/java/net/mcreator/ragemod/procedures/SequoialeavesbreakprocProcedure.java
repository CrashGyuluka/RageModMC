	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SILK_TOUCH,
			...
			if (world instanceof Level _level && !_level.isClientSide()) {
				ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(RagemodModBlocks.SEQUOIA_LEAVES));
			...
			if (Math.random() < 0.1) {
				if (world instanceof Level _level && !_level.isClientSide()) {
					ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(RagemodModBlocks.SEQUOIA_SAPLING));
					entityToSpawn.setPickUpDelay(10);
					_level.addFreshEntity(entityToSpawn);
				}
			}
