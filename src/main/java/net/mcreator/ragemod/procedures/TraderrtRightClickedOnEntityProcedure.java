...
import net.mcreator.ragemod.gui.RttraderguiGui;
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				RagemodMod.LOGGER.warn("Failed to load dependency x for procedure TraderrtRightClickedOnEntity!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				RagemodMod.LOGGER.warn("Failed to load dependency y for procedure TraderrtRightClickedOnEntity!");
			return;
		}
			...
			if (_ent instanceof ServerPlayerEntity) {
				...
				NetworkHooks.openGui((ServerPlayerEntity) _ent, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Rttradergui");
					}

					@Override
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						return new RttraderguiGui.GuiContainerMod(id, inventory, new PacketBuffer(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
		}
