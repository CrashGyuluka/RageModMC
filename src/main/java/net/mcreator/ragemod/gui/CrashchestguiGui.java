...
public class CrashchestguiGui extends RagemodModElements.ModElement {
	...
	private static class ContainerRegisterHandler {
		...
		public void registerContainer(RegistryEvent.Register<ContainerType<?>> event) {
			event.getRegistry().register(containerType.setRegistryName("crashchestgui"));
		}
	}

	@OnlyIn(Dist.CLIENT)
	public void initElements() {
		DeferredWorkQueue.runLater(() -> ScreenManager.registerFactory(containerType, CrashchestguiGuiWindow::new));
	}
	...
	public static class GuiContainerMod extends Container implements Supplier<Map<Integer, Slot>> {
		...
		public ItemStack transferStackInSlot(PlayerEntity playerIn, int index) {
			...
			if (slot != null && slot.getHasStack()) {
				...
				if (index < 56) {
					if (!this.mergeItemStack(itemstack1, 56, this.inventorySlots.size(), true)) {
						return ItemStack.EMPTY;
					}
					slot.onSlotChange(itemstack1, itemstack);
				} else if (!this.mergeItemStack(itemstack1, 0, 56, false)) {
					if (index < 56 + 27) {
						if (!this.mergeItemStack(itemstack1, 56 + 27, this.inventorySlots.size(), true)) {
							return ItemStack.EMPTY;
						}
					} else {
						if (!this.mergeItemStack(itemstack1, 56, 56 + 27, false)) {
							return ItemStack.EMPTY;
						}
					}
					return ItemStack.EMPTY;
