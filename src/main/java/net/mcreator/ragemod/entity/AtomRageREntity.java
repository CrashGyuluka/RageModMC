...
public class AtomRageREntity extends Monster {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("ragemod:toxic_waste"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(RagemodModEntities.ATOM_RAGE_R, 2, 1, 2));
	}

	public AtomRageREntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(RagemodModEntities.ATOM_RAGE_R, world);
	}
	...
	public static void init() {
		SpawnPlacements.register(RagemodModEntities.ATOM_RAGE_R, SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL
						&& Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(RagemodModEntities.ATOM_RAGE_R, 180);
	}
